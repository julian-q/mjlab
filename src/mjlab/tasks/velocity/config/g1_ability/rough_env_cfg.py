...
from mjlab.asset_zoo.robots.unitree_g1_ability.g1_ability_constants import (
  G1_ABILITY_ACTION_SCALE,
  G1_ABILITY_ROBOT_CFG,
)
...
@dataclass
class UnitreeG1AbilityRoughEnvCfg(LocomotionVelocityEnvCfg):
  def __post_init__(self):
    super().__post_init__()

    self.scene.entities = {"robot": replace(G1_ABILITY_ROBOT_CFG)}
    ...
    # Actions.
    self.actions.joint_pos.scale = G1_ABILITY_ACTION_SCALE
...
@dataclass
class UnitreeG1AbilityRoughEnvCfg_PLAY(UnitreeG1AbilityRoughEnvCfg):
  def __post_init__(self):
    super().__post_init__()
